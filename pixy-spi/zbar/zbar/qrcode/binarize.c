...
void qr_wiener_filter(unsigned char *_img,int _width,int _height){
  ...
  m_buf[0]=(unsigned *)__malloc((_width+4<<3)*sizeof(*m_buf));
  sn2_buf[0]=(unsigned *)__malloc((_width+4<<3)*sizeof(*sn2_buf));
  for(y=1;y<8;y++){
...
void qr_wiener_filter(unsigned char *_img,int _width,int _height){
  ...
  int                 y;
  if(_width<=0||_height<=0)return;
  m_buf[0]=(unsigned *)__malloc((_width+2<<2)*sizeof(*m_buf));
  sn2_buf[0]=(unsigned *)__malloc((_width+2<<2)*sizeof(*sn2_buf));
  for(y=1;y<4;y++){
...
static void qr_sauvola_mask(unsigned char *_mask,unsigned *_b,int *_nb,
 const unsigned char *_img,int _width,int _height){
  ...
  if(_width>0&&_height>0){
    ...
    windw=1<<logwindw;
    windh=1<<logwindh;
    col_sums=(unsigned *)__malloc(_width*sizeof(*col_sums));
    col2_sums=(unsigned *)__malloc(_width*sizeof(*col2_sums));
...
static void qr_interpolate_background(unsigned char *_dst,
 ...
 int _width,int _height,unsigned _b,int _nb){
  ...
  if(_width>0&&_height>0){
    ...
    windh=1<<logwindh;
    col_sums=(unsigned *)__malloc(_width*sizeof(*col_sums));
    ncol_sums=(unsigned *)__malloc(_width*sizeof(*ncol_sums));
    /*Initialize sums down each column.*/
    for(x=0;x<_width;x++){
      if(!_mask[x]){
        g=_img[x];
...
void qr_binarize(unsigned char *_img,int _width,int _height){
  ...
  mask=(unsigned char *)__malloc(_width*_height*sizeof(*mask));
  ...
  background=(unsigned char *)__malloc(_width*_height*sizeof(*mask));
...
int main(int _argc,char **_argv){
  ...
  /*width=1182;
  height=1181;
  img=(unsigned char *)__malloc(width*height*sizeof(*img));
